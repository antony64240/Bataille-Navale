...
public class Croiseur extends Navire{
	...
	public void pushAroundCase(boolean vertical) {
		if(this.Orientation) {
			for(CaseNavire e :this.parts) {
				if(e.getX()>0) {AroundNavire.add(new CaseNavire(e.getX()-1,e.getY(),8));}	
				if(e.getX()<9) {AroundNavire.add(new CaseNavire(e.getX()+1,e.getY(),8));}
			}
		...
		if(!this.Orientation) {
			for(CaseNavire e :this.parts) {
				if(e.getY()>0) {AroundNavire.add(new CaseNavire(e.getX(),e.getY()-1,8));}
				if(e.getY()<9) {AroundNavire.add(new CaseNavire(e.getX(),e.getY()+1,8));}
			}
